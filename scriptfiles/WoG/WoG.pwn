...
public OnPlayerDisconnect(playerid,reason)
{
	...
	{
		do pcount--;
		while((IsValidNPC(playerid) || (IsPlayerConnected(pcount) == 0)) && (pcount > 0));
	}
    // pickups
    PlayerOnPickup[playerid] = 0;
...
#endif
#if defined _guide_included
	RegisterText(text,'?',Guide,GUIDE_REQUEST_TEXT);
#endif
...
{
#if defined _player_included
if(oldstate == PLAYER_STATE_DRIVER || PLAYER_STATE_PASSENGER) StopAudioStreamForPlayer(playerid);
#endif
#if defined _fuel_included
