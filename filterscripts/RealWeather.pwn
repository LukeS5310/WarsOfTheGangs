...
{
	if(!strcmp(cmdtext, "/winfo", true,6))	//просмотр прогноза
	{
	    if(!strval(Config[3][valueParameter]))return 1;
	    ...
	    #pragma unused timeDay
	    #pragma unused Cloudiness
	    #pragma unused Precipitation
	    #pragma unused Wind
	    #pragma unused Weekday
		return 1;
	}
	if(!strcmp(cmdtext, "/wdebug", true,8)) //системная информация
	{
	...
	if(!strcmp(cmdtext, "/wset", true, 5))		//установить погоду ручками (автопогода отключиться)
	{
	    if(!IsPlayerAdmin(playerid))return 1;
